...
コードフェンス（`` ``` `` / `~~~`）内では、JS テンプレートリテラルと同型の `${expr}` を展開します。
...
- `@hg` 内テンプレートリテラルの `${}` と同じ許可式（`.toLocaleString` / `.length` 可、component 呼び出し不可）
- フェンス外の `${…}` はリテラルのまま
- `\${` は展開しない
- `{{ }}` / `{{{ }}}` とは別系統（マスタッシュはフェンス内でも従来どおり展開）

詳細は [hyogen ブロック](/ja/syntax/hg-blocks) も参照。
