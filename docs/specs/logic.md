...
詳細構文は [dsl.md](./dsl.md)。方向性と制限のみここに固定する。
...
| 領域 | できること |
|------|------------|
| hyogen ブロック（`@hg` / `@@`） | 本仕様で定義したロジックを **実行**（宣言・代入・`for`・`echo`・`component ... as` など）。`if` / `each` は step 5 で本文展開 |
| `{{ }}` / `{{{ }}}` | **式のみ**（三項・パイプ・メンバアクセス、登録済み **component** 呼び出し、`.toLocaleString`、許可プロパティ `.length` など。文・each は不可） |
