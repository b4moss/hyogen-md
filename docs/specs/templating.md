...
- **見出し階層**: 挿入位置の直前見出しレベル `L` に合わせ、取り込み側の見出しを `min(6, H+L)` にシフトする（ネストは累積。ATX + Setext。フェンス内は除外。詳細は下記）
...
`include` / `component` を親へ差し込むとき、挿入位置の **直前見出し** に合わせて取り込み側の見出しレベルをずらす。

| 項目 | 内容 |
|------|------|
| 対象 | `include` と `component` |
| 取り込み側の書き方 | 見出しは常に `#`（レベル 1）から始める |
| シフト | 直前見出しレベルを `L`、著者レベルを `H` として `H' = min(6, H + L)`（ルートの累積オフセットは挿入境界の合成で実現） |
| ネスト | オフセットは累積する |
| 上限 | 常に `######` が最深（クランプ。エラー・警告なし） |
| 判定 | ATX（`#`〜`######`）と Setext（`===` / `---`）。Setext はシフト時 ATX に正規化 |
| 除外 | コードフェンス（`` ``` `` / `~~~`）内は見出しとみなさない |
