...
- `{{ }}` 内で呼べる関数は **ユーザー登録関数**（例: `component ... as name` の `name(...)`）に限る
- メソッドは **`.toLocaleString(...)` のみ**許可（引数は **JS に準拠**。バリデーションなし）
- 許可プロパティ **`.length`**（配列・文字列。`length()` 呼び出しは不可）
- デフォルト値をパイプで書ける:
...
- フェンス外の `${…}` はリテラルのまま
- `\${` は展開しない（リテラル `${`）
- `{{ }}` / `{{{ }}}` とは別系統（マスタッシュの挙動は変更しない。フェンス内でも `{{ }}` は従来どおり展開する）
